...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.galleryCollection.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.gallery-collections.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="collection_name">{{ trans('cruds.galleryCollection.fields.collection_name') }}</label>
                <input class="form-control {{ $errors->has('collection_name') ? 'is-invalid' : '' }}" type="text" name="collection_name" id="collection_name" value="{{ old('collection_name', '') }}" required>
