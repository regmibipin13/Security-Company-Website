...
@section('content')
    <div class="card">
        <div class="card-header">
            {{ trans('global.create') }} {{ trans('cruds.satisfaction.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route('admin.satisfactions.store') }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('employee_id') ? 'is-invalid' : '' }}" type="text"
                        name="employee_id" id="employee" value="{{ old('employee_id', '') }}">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('rate') ? 'is-invalid' : '' }}" type="number"
                        name="rate" id="rate" value="{{ old('rate', '') }}" step="0.01" min="1"
                        max="5">
                    @if ($errors->has('rate'))
                        <div class="invalid-feedback">
                            {{ $errors->first('rate') }}
